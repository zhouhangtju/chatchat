...
# from langchain.vectorstores.faiss import FAISSWithMetadata
from vector_stores import FAISSWithMetadata
from langchain.docstore.in_memory import InMemoryDocstore
from langchain.schema import Document
import os
from langchain.schema import Document
from configs.kb_config import METADATA_QUERY
...
class _FaissPool(CachePool):
    def new_vector_store(
        ...
        # vector_store = FAISSWithMetadata.from_documents([doc], embeddings, distance_strategy="METRIC_INNER_PRODUCT")
        if METADATA_QUERY:
            vector_store = FAISSWithMetadata.from_documents([doc], embeddings, distance_strategy="EUCLIDEAN_DISTANCE")
        else:
            vector_store = FAISS.from_documents([doc], embeddings, distance_strategy="EUCLIDEAN_DISTANCE")
    ...
    def load_vector_store(
        ...
        if cache is None:
            ...
            with item.acquire(msg="初始化"):
                ...
                #FAISSWithMetadata
                if os.path.isfile(os.path.join(vs_path, "index_content.faiss")) or os.path.isfile(os.path.join(vs_path, "index.faiss")):
                    if METADATA_QUERY:
                        if os.path.isfile(os.path.join(vs_path, "index_content.faiss")):
                            embeddings = self.load_kb_embeddings(kb_name=kb_name, embed_device=embed_device, default_embed_model=embed_model)
                            vector_store = FAISSWithMetadata.load_local(vs_path, embeddings, distance_strategy="EUCLIDEAN_DISTANCE")
                    elif not METADATA_QUERY:
                        if os.path.isfile(os.path.join(vs_path, "index.faiss")):
                            embeddings = self.load_kb_embeddings(kb_name=kb_name, embed_device=embed_device, default_embed_model=embed_model)
                            vector_store = FAISS.load_local(vs_path, embeddings, distance_strategy="EUCLIDEAN_DISTANCE")
                ...
                #     # EUCLIDEAN_DISTANCE 距离越小，相似度越高； MAX_INNER_PRODUCT 距离越大，相似度越高
                #     # vector_store = FAISS.load_local(vs_path, embeddings, distance_strategy="EUCLIDEAN_DISTANCE")
                #     vector_store = FAISSWithMetadata.load_local(vs_path, embeddings, distance_strategy="EUCLIDEAN_DISTANCE")
