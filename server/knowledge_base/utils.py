...
from document_loaders.myDocxLoadAndSplitter import MyDocxLoader
from text_splitter import MyMarkdownHeaderTextSplitter
...
def get_loader(loader_name: str, file_path: str, loader_kwargs: Dict = None):
    ...
    try:
        if loader_name in ["RapidOCRPDFLoader", "RapidOCRLoader", "FilteredCSVLoader",
                           "RapidOCRDocLoader", "RapidOCRPPTLoader","MyDocxLoader"]:
...
def make_text_splitter(
    ...
    try:
        if splitter_name == "MarkdownHeaderTextSplitter":  # MarkdownHeaderTextSplitter特殊判定
            ...
            text_splitter = MyMarkdownHeaderTextSplitter(
                headers_to_split_on=headers_to_split_on)           
        else:
...
class KnowledgeFile:
    ...
    def docs2texts(
        ...
        if self.ext not in [".csv"]:
            ...
            if self.ext==".md":
                with open(docs[0].metadata["source"],"r",encoding="utf-8") as f:
                    content = f.read()
                # docs = text_splitter.split_text(docs[0].page_content)
                docs = text_splitter.split_text(content)
            if self.ext==".pdf":
                loader = PyPDFLoader(docs[0].metadata["source"])
                docs = loader.load_and_split()
            if self.ext==".docx" or self.ext==".doc":
                return docs
