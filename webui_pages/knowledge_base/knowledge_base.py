...
def knowledge_base_page(api: ApiRequest, is_lite: bool = None):
    ...
    if selected_kb == "新建知识库":
        ...
        with st.expander(
            ...
            cols = st.columns(3)
            chunk_size = cols[0].number_input("单段文本最大长度：", 1, 8192, CHUNK_SIZE)
        ...
        with st.sidebar:
            ...
            search_button = st.button("搜索文档")

            # 定义一个占位符来放置搜索结果
            search_results_placeholder = st.empty()

            if search_button:
                if keyword and selected_rows:
                    file_name = selected_rows[0]["file_name"]
                    # 调用 API 进行文档搜索
                    search_results = api.search_kb_docs(
